...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> recuperer() async{
    ...
    if(login == true) {
      ...
      final snapshot = await ref.child('administrateur/$card').get();
      if (snapshot.exists) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Acceuil Restaurant Admin", style: TextStyle(color: Colors.white),),
      ),
      body: Center(
  ...
  Future<void> base() async{
    final ref = FirebaseDatabase.instance.ref();
    final snapshot = await ref.child('administrateur/${cardNumber.text}').get();
    if (snapshot.exists) {
